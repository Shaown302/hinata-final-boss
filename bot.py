...
async def cmd_game_riddle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = "Generate a fun riddle for a game. Return requested format: RIDDLE: [the riddle text] ANSWER: [the one word answer]"
    async with httpx.AsyncClient() as client:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=welcome_text,
        reply_markup=get_main_menu(page=1, user_id=user.id),
        parse_mode="HTML"
    )

def get_main_menu(page=1, user_id=None):
    """Generates paginated menu buttons (10 per page)."""
    # Define all available feature buttons
    all_buttons = [
        # Page 1: AI & Primary Tools (mostly)
        ...
        InlineKeyboardButton("✍️ Grammar Check", callback_data="btn_grammar"),
        InlineKeyboardButton("💡 AI Riddle", callback_data="btn_riddle"),
        ...
        InlineKeyboardButton("🔢 Number Guess", callback_data="btn_guess"),
    ...
    page_items = all_buttons[start:end]
    ...
    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("⬅️ Previous", callback_data=f"menu_page:{page-1}"))
    if page < total_pages:
        nav.append(InlineKeyboardButton("Next ➡️", callback_data=f"menu_page:{page+1}"))
    if nav:
        kb.append(nav)
    
    # Add Admin button if owner on every page for convenience
    if is_owner(user_id):
        kb.append([InlineKeyboardButton("⚙️ Admin Menu", callback_data="btn_admin")])
...
def clear_states(ud):
    ...
    for key in [AWAIT_GEMINI, AWAIT_DEEPSEEK, AWAIT_FLIRT, AWAIT_INSTA, AWAIT_USERINFO, AWAIT_FF, AWAIT_FFGUILD, AWAIT_CODE, AWAIT_DL, AWAIT_QRGEN, AWAIT_BGREM, AWAIT_FFLIKE, AWAIT_TRANSLATE, AWAIT_SUMMARIZE, AWAIT_GRAMMAR, AWAIT_BAN, AWAIT_UNBAN, AWAIT_GUESS, AWAIT_RIDDLE]:
        ud.pop(key, None)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("menu_page:"):
        page = int(data.split(":")[1])
        await query.edit_message_reply_markup(reply_markup=get_main_menu(page, query.from_user.id))
    elif data == "btn_admin":
